defmodule ApiChecker.PeriodicTask.Validator do
  ...
  def validate(%PeriodicTask{} = task) do
    with \
      :ok <- run_validation(task, :frequency_in_seconds, &is_pos_integer?/1, "must be a positive integer"),
      ...
      :ok <- run_validation(task, :list_of_time_range, &is_list_of_time_ranges?/1, "must be a list of valid time ranges"),
  ...
  iex> Validator.is_valid_url?("http://realtime.mbta.com/developer/api/v2/vehiclesbyroutes?api_key=9yaXQLoRF02t3UfWjxyiDQ&format=json&routes=1")
  true
